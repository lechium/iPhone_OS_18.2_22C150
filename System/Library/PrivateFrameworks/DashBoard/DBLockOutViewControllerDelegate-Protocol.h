//
//     Generated by classdump-c 4.2.0 (64 bit) (iOS port by DreamDevLost, Updated by Kevin Bradley.)(Debug version compiled Dec 27 2024 14:18:54).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <DashBoard/NSObject-Protocol.h>

@class DBLockOutViewController, UIView;

@protocol DBLockOutViewControllerDelegate <NSObject>
- (void)lockOutViewControllerWantsToDismiss:(DBLockOutViewController *)arg1;
- (void)lockOutViewController:(DBLockOutViewController *)arg1 didTapButtonOfType:(unsigned long long)arg2;
- (UIView *)wallpaperViewForLockOutViewController:(DBLockOutViewController *)arg1;
@end

