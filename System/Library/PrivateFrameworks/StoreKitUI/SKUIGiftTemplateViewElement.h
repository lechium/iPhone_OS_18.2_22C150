//
//     Generated by classdump-c 4.2.0 (64 bit) (iOS port by DreamDevLost, Updated by Kevin Bradley.)(Debug version compiled Dec 27 2024 14:18:54).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import "SKUIViewElement.h"

@class NSString, SKUIItem;

__attribute__((visibility("hidden")))
@interface SKUIGiftTemplateViewElement : SKUIViewElement
{
    long long _giftType;	// 8 = 0x8
    NSString *_productBuyParams;	// 16 = 0x10
    long long _productItemIdentifier;	// 24 = 0x18
}

- (void).cxx_destruct;	// IMP=0x000000000021f364
@property(readonly, nonatomic) long long giftType; // @synthesize giftType=_giftType;
- (id)applyUpdatesWithElement:(id)arg1;	// IMP=0x000000000021f27d
@property(readonly, nonatomic) SKUIItem *productItem;
- (id)initWithDOMElement:(id)arg1 parent:(id)arg2 elementFactory:(id)arg3;	// IMP=0x000000000021e628

@end

