//
//     Generated by classdump-c 4.2.0 (64 bit) (iOS port by DreamDevLost, Updated by Kevin Bradley.)(Debug version compiled Dec 27 2024 14:18:54).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

__attribute__((visibility("hidden")))
@interface _ICNANoteReportToAccount : NSObject
{
    float _noteTwoFactorMatrix[10][10];	// 8 = 0x8
    int _noteCollaborationMatrix[3][3];	// 408 = 0x198
    unsigned long long _countOfNotesWithDocScan;	// 448 = 0x1c0
    unsigned long long _countOfNotesWithInlineDrawingV1;	// 456 = 0x1c8
    unsigned long long _countOfNotesWithInlineDrawingV2;	// 464 = 0x1d0
    unsigned long long _countOfNotesWithFullscreenDrawing;	// 472 = 0x1d8
    unsigned long long _countOfNotesWithTable;	// 480 = 0x1e0
    unsigned long long _countOfNotesWithRichURL;	// 488 = 0x1e8
    unsigned long long _countOfNotesWithMapLink;	// 496 = 0x1f0
    unsigned long long _countOfNotesWithOtherAttachments;	// 504 = 0x1f8
    unsigned long long _countOfNotesWithPaperKit;	// 512 = 0x200
    unsigned long long _countOfNotesWithDeepLink;	// 520 = 0x208
    unsigned long long _countOfNotesWithPaperDocument;	// 528 = 0x210
    unsigned long long _countOfNotesWithPDF;	// 536 = 0x218
    unsigned long long _countOfNotesWithNoteLink;	// 544 = 0x220
    unsigned long long _countOfNotesWithFolderLink;	// 552 = 0x228
    unsigned long long _countOfNotesWithMathUsage;	// 560 = 0x230
    unsigned long long _countOfNotesWithAttachmentAudio;	// 568 = 0x238
    unsigned long long _countOfNotesWithAttachmentAudioTranscript;	// 576 = 0x240
    unsigned long long _countOfNotesWithAttachmentAudioSummary;	// 584 = 0x248
    unsigned long long _countOfAudioAttachments;	// 592 = 0x250
    unsigned long long _countOfAudioAttachmentsRecordedInNotes;	// 600 = 0x258
    unsigned long long _countOfAudioAttachmentsWithAppendedAudio;	// 608 = 0x260
    unsigned long long _countOfNotesWithTags;	// 616 = 0x268
    unsigned long long _countOfNotesWithMentions;	// 624 = 0x270
    unsigned long long _scrapPaperCount;	// 632 = 0x278
    unsigned long long _countOfScrapPapersWithTags;	// 640 = 0x280
    unsigned long long _countOfScrapPapersWithMentions;	// 648 = 0x288
    unsigned long long _countOfPinnedScrapPapers;	// 656 = 0x290
    unsigned long long _countOfLockedScrapPapers;	// 664 = 0x298
    unsigned long long _countOfCollaboratedScrapPapers;	// 672 = 0x2a0
    unsigned long long _countOfScrapPapersWithDocScan;	// 680 = 0x2a8
    unsigned long long _countOfScrapPapersWithInlineDrawingV1;	// 688 = 0x2b0
    unsigned long long _countOfScrapPapersWithInlineDrawingV2;	// 696 = 0x2b8
    unsigned long long _countOfScrapPapersWithFullscreenDrawing;	// 704 = 0x2c0
    unsigned long long _countOfScrapPapersWithTables;	// 712 = 0x2c8
    unsigned long long _countOfScrapPapersWithRichUrl;	// 720 = 0x2d0
    unsigned long long _countOfScrapPapersWithMapLink;	// 728 = 0x2d8
    unsigned long long _countOfScrapPapersWithOtherAttachments;	// 736 = 0x2e0
    unsigned long long _countOfScrapPapersWithPaperKit;	// 744 = 0x2e8
    unsigned long long _countOfScrapPapersWithDeepLink;	// 752 = 0x2f0
    unsigned long long _countOfNotes;	// 760 = 0x2f8
    unsigned long long _countOfChecklists;	// 768 = 0x300
    unsigned long long _countOfNotesWithChecklists;	// 776 = 0x308
    unsigned long long _countOfNotesWithCollapsedSections;	// 784 = 0x310
    unsigned long long _countOfPinnedNotes;	// 792 = 0x318
    unsigned long long _countOfLockedNotes;	// 800 = 0x320
    unsigned long long _countOfV1LockedNotes;	// 808 = 0x328
    unsigned long long _countOfV2LockedNotes;	// 816 = 0x330
    unsigned long long _countOfLockedNotesWithDivergedMode;	// 824 = 0x338
    unsigned long long _countOfLockedNotesWithDivergedPassword;	// 832 = 0x340
}

@property(nonatomic) unsigned long long countOfLockedNotesWithDivergedPassword; // @synthesize countOfLockedNotesWithDivergedPassword=_countOfLockedNotesWithDivergedPassword;
@property(nonatomic) unsigned long long countOfLockedNotesWithDivergedMode; // @synthesize countOfLockedNotesWithDivergedMode=_countOfLockedNotesWithDivergedMode;
@property(nonatomic) unsigned long long countOfV2LockedNotes; // @synthesize countOfV2LockedNotes=_countOfV2LockedNotes;
@property(nonatomic) unsigned long long countOfV1LockedNotes; // @synthesize countOfV1LockedNotes=_countOfV1LockedNotes;
@property(nonatomic) unsigned long long countOfLockedNotes; // @synthesize countOfLockedNotes=_countOfLockedNotes;
@property(nonatomic) unsigned long long countOfPinnedNotes; // @synthesize countOfPinnedNotes=_countOfPinnedNotes;
@property(nonatomic) unsigned long long countOfNotesWithCollapsedSections; // @synthesize countOfNotesWithCollapsedSections=_countOfNotesWithCollapsedSections;
@property(nonatomic) unsigned long long countOfNotesWithChecklists; // @synthesize countOfNotesWithChecklists=_countOfNotesWithChecklists;
@property(nonatomic) unsigned long long countOfChecklists; // @synthesize countOfChecklists=_countOfChecklists;
@property(nonatomic) unsigned long long countOfNotes; // @synthesize countOfNotes=_countOfNotes;
@property(nonatomic) unsigned long long countOfScrapPapersWithDeepLink; // @synthesize countOfScrapPapersWithDeepLink=_countOfScrapPapersWithDeepLink;
@property(nonatomic) unsigned long long countOfScrapPapersWithPaperKit; // @synthesize countOfScrapPapersWithPaperKit=_countOfScrapPapersWithPaperKit;
@property(nonatomic) unsigned long long countOfScrapPapersWithOtherAttachments; // @synthesize countOfScrapPapersWithOtherAttachments=_countOfScrapPapersWithOtherAttachments;
@property(nonatomic) unsigned long long countOfScrapPapersWithMapLink; // @synthesize countOfScrapPapersWithMapLink=_countOfScrapPapersWithMapLink;
@property(nonatomic) unsigned long long countOfScrapPapersWithRichUrl; // @synthesize countOfScrapPapersWithRichUrl=_countOfScrapPapersWithRichUrl;
@property(nonatomic) unsigned long long countOfScrapPapersWithTables; // @synthesize countOfScrapPapersWithTables=_countOfScrapPapersWithTables;
@property(nonatomic) unsigned long long countOfScrapPapersWithFullscreenDrawing; // @synthesize countOfScrapPapersWithFullscreenDrawing=_countOfScrapPapersWithFullscreenDrawing;
@property(nonatomic) unsigned long long countOfScrapPapersWithInlineDrawingV2; // @synthesize countOfScrapPapersWithInlineDrawingV2=_countOfScrapPapersWithInlineDrawingV2;
@property(nonatomic) unsigned long long countOfScrapPapersWithInlineDrawingV1; // @synthesize countOfScrapPapersWithInlineDrawingV1=_countOfScrapPapersWithInlineDrawingV1;
@property(nonatomic) unsigned long long countOfScrapPapersWithDocScan; // @synthesize countOfScrapPapersWithDocScan=_countOfScrapPapersWithDocScan;
@property(nonatomic) unsigned long long countOfCollaboratedScrapPapers; // @synthesize countOfCollaboratedScrapPapers=_countOfCollaboratedScrapPapers;
@property(nonatomic) unsigned long long countOfLockedScrapPapers; // @synthesize countOfLockedScrapPapers=_countOfLockedScrapPapers;
@property(nonatomic) unsigned long long countOfPinnedScrapPapers; // @synthesize countOfPinnedScrapPapers=_countOfPinnedScrapPapers;
@property(nonatomic) unsigned long long countOfScrapPapersWithMentions; // @synthesize countOfScrapPapersWithMentions=_countOfScrapPapersWithMentions;
@property(nonatomic) unsigned long long countOfScrapPapersWithTags; // @synthesize countOfScrapPapersWithTags=_countOfScrapPapersWithTags;
@property(nonatomic) unsigned long long scrapPaperCount; // @synthesize scrapPaperCount=_scrapPaperCount;
@property(nonatomic) unsigned long long countOfNotesWithMentions; // @synthesize countOfNotesWithMentions=_countOfNotesWithMentions;
@property(nonatomic) unsigned long long countOfNotesWithTags; // @synthesize countOfNotesWithTags=_countOfNotesWithTags;
@property(nonatomic) unsigned long long countOfAudioAttachmentsWithAppendedAudio; // @synthesize countOfAudioAttachmentsWithAppendedAudio=_countOfAudioAttachmentsWithAppendedAudio;
@property(nonatomic) unsigned long long countOfAudioAttachmentsRecordedInNotes; // @synthesize countOfAudioAttachmentsRecordedInNotes=_countOfAudioAttachmentsRecordedInNotes;
@property(nonatomic) unsigned long long countOfAudioAttachments; // @synthesize countOfAudioAttachments=_countOfAudioAttachments;
@property(nonatomic) unsigned long long countOfNotesWithAttachmentAudioSummary; // @synthesize countOfNotesWithAttachmentAudioSummary=_countOfNotesWithAttachmentAudioSummary;
@property(nonatomic) unsigned long long countOfNotesWithAttachmentAudioTranscript; // @synthesize countOfNotesWithAttachmentAudioTranscript=_countOfNotesWithAttachmentAudioTranscript;
@property(nonatomic) unsigned long long countOfNotesWithAttachmentAudio; // @synthesize countOfNotesWithAttachmentAudio=_countOfNotesWithAttachmentAudio;
@property(nonatomic) unsigned long long countOfNotesWithMathUsage; // @synthesize countOfNotesWithMathUsage=_countOfNotesWithMathUsage;
@property(nonatomic) unsigned long long countOfNotesWithFolderLink; // @synthesize countOfNotesWithFolderLink=_countOfNotesWithFolderLink;
@property(nonatomic) unsigned long long countOfNotesWithNoteLink; // @synthesize countOfNotesWithNoteLink=_countOfNotesWithNoteLink;
@property(nonatomic) unsigned long long countOfNotesWithPDF; // @synthesize countOfNotesWithPDF=_countOfNotesWithPDF;
@property(nonatomic) unsigned long long countOfNotesWithPaperDocument; // @synthesize countOfNotesWithPaperDocument=_countOfNotesWithPaperDocument;
@property(nonatomic) unsigned long long countOfNotesWithDeepLink; // @synthesize countOfNotesWithDeepLink=_countOfNotesWithDeepLink;
@property(nonatomic) unsigned long long countOfNotesWithPaperKit; // @synthesize countOfNotesWithPaperKit=_countOfNotesWithPaperKit;
@property(nonatomic) unsigned long long countOfNotesWithOtherAttachments; // @synthesize countOfNotesWithOtherAttachments=_countOfNotesWithOtherAttachments;
@property(nonatomic) unsigned long long countOfNotesWithMapLink; // @synthesize countOfNotesWithMapLink=_countOfNotesWithMapLink;
@property(nonatomic) unsigned long long countOfNotesWithRichURL; // @synthesize countOfNotesWithRichURL=_countOfNotesWithRichURL;
@property(nonatomic) unsigned long long countOfNotesWithTable; // @synthesize countOfNotesWithTable=_countOfNotesWithTable;
@property(nonatomic) unsigned long long countOfNotesWithFullscreenDrawing; // @synthesize countOfNotesWithFullscreenDrawing=_countOfNotesWithFullscreenDrawing;
@property(nonatomic) unsigned long long countOfNotesWithInlineDrawingV2; // @synthesize countOfNotesWithInlineDrawingV2=_countOfNotesWithInlineDrawingV2;
@property(nonatomic) unsigned long long countOfNotesWithInlineDrawingV1; // @synthesize countOfNotesWithInlineDrawingV1=_countOfNotesWithInlineDrawingV1;
@property(nonatomic) unsigned long long countOfNotesWithDocScan; // @synthesize countOfNotesWithDocScan=_countOfNotesWithDocScan;
- (unsigned long long)countOfNotesWithCollaborationStatus:(long long)arg1 collaborationType:(long long)arg2;	// IMP=0x000000000000d9c1
- (unsigned long long)countOfNotesWithFactor:(long long)arg1 andFactor:(long long)arg2;	// IMP=0x000000000000d977
- (void)completeTwoFactorMatrixReportingForCurrentNote;	// IMP=0x000000000000d925
- (void)updateNoteCollaborationMatrixWithCollaborationStatus:(id)arg1 collaborationType:(id)arg2;	// IMP=0x000000000000d873
- (void)updateNoteTwoFactorMatrixWithIndex:(long long)arg1;	// IMP=0x000000000000d7f9
- (id)init;	// IMP=0x000000000000d7aa

@end

