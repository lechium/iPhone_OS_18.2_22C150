//
//     Generated by classdump-c 4.2.0 (64 bit) (iOS port by DreamDevLost, Updated by Kevin Bradley.)(Debug version compiled Dec 27 2024 14:18:54).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSProgress;

__attribute__((visibility("hidden")))
@interface HDCloudSyncObserverTaskServerStoredProgress : NSObject
{
    NSProgress *_progress;	// 8 = 0x8
    CDUnknownBlockType _invalidationBlock;	// 16 = 0x10
}

- (void).cxx_destruct;	// IMP=0x00000000001d248d
@property(copy, nonatomic) CDUnknownBlockType invalidationBlock; // @synthesize invalidationBlock=_invalidationBlock;
@property(retain, nonatomic) NSProgress *progress; // @synthesize progress=_progress;
- (void)invalidate;	// IMP=0x00000000001d23e8
- (id)initWithProgress:(id)arg1;	// IMP=0x00000000001d236b
- (id)init;	// IMP=0x00000000001d22f1

@end

