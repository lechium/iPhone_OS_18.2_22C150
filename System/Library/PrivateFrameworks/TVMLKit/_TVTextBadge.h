//
//     Generated by classdump-c 4.2.0 (64 bit) (iOS port by DreamDevLost, Updated by Kevin Bradley.)(Debug version compiled Dec 27 2024 14:18:54).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <UIKit/UIView.h>

@class NSAttributedString, NSString, UIColor;

__attribute__((visibility("hidden")))
@interface _TVTextBadge : UIView
{
    NSAttributedString *_text;	// 8 = 0x8
    NSString *_type;	// 16 = 0x10
    UIColor *_tintColor;	// 24 = 0x18
    double _cornerRadius;	// 32 = 0x20
    double _lineWidth;	// 40 = 0x28
}

+ (id)textBadgeViewWithElement:(id)arg1 existingView:(id)arg2;	// IMP=0x006000000008ab96
- (void).cxx_destruct;	// IMP=0x000000000008b322
@property(nonatomic) double lineWidth; // @synthesize lineWidth=_lineWidth;
@property(nonatomic) double cornerRadius; // @synthesize cornerRadius=_cornerRadius;
@property(retain, nonatomic) UIColor *tintColor; // @synthesize tintColor=_tintColor;
@property(retain, nonatomic) NSString *type; // @synthesize type=_type;
@property(retain, nonatomic) NSAttributedString *text; // @synthesize text=_text;
- (void)drawRect:(struct CGRect)arg1;	// IMP=0x000000000008b056
- (struct CGSize)sizeThatFits:(struct CGSize)arg1;	// IMP=0x000000000008af3b

@end

