//
//     Generated by classdump-c 4.2.0 (64 bit) (iOS port by DreamDevLost, Updated by Kevin Bradley.)(Debug version compiled Dec 27 2024 14:18:54).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@class MISSING_TYPE, NSDate, NSNumber, _TtC8MapsSync19MapsSyncCommunityID, _TtC8MapsSync27MapsSyncAnonymousCredential;

@interface _TtC8MapsSync28MapsSyncMutableReviewedPlace
{
    MISSING_TYPE *_proxyObject;	// 16 = 0x10
}

- (void).cxx_destruct;	// IMP=0x00000000001751e0
@property(nonatomic, retain) _TtC8MapsSync19MapsSyncCommunityID *communityID;
@property(nonatomic, retain) _TtC8MapsSync27MapsSyncAnonymousCredential *anonymousCredential;
- (void)setMapItemIdentifier:(id)arg1;	// IMP=0x0000000000174590
@property(nonatomic) short version;
@property(nonatomic) int uploadedPhotosCount;
@property(nonatomic, retain) NSNumber *rating;
@property(nonatomic) _Bool hasUserReviewed;
@property(nonatomic, copy) NSDate *lastSuggestedReviewDate;
@property(nonatomic) unsigned long long muid;
- (id)initWithProxyObject:(id)arg1;	// IMP=0x0000000000173620

@end

