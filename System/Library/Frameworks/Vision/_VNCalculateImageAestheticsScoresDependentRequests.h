//
//     Generated by classdump-c 4.2.0 (64 bit) (iOS port by DreamDevLost, Updated by Kevin Bradley.)(Debug version compiled Dec 27 2024 14:18:54).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class VNClassifyImageAestheticsRequest, VNClassifyJunkImageRequest;

__attribute__((visibility("hidden")))
@interface _VNCalculateImageAestheticsScoresDependentRequests : NSObject
{
    VNClassifyImageAestheticsRequest *_oldAestheticsRequest;	// 8 = 0x8
    VNClassifyJunkImageRequest *_junkRequest;	// 16 = 0x10
}

- (void).cxx_destruct;	// IMP=0x00000000003afd1b
@property(readonly) VNClassifyJunkImageRequest *junkRequest; // @synthesize junkRequest=_junkRequest;
@property(readonly) VNClassifyImageAestheticsRequest *oldAestheticsRequest; // @synthesize oldAestheticsRequest=_oldAestheticsRequest;
- (id)requests;	// IMP=0x00000000003afc87
- (id)initWithRevision:(unsigned long long)arg1 error:(id *)arg2;	// IMP=0x00000000003afb92

@end

