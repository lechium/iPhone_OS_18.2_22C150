//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <ProtocolBuffer/PBCodable.h>

@class ALCLNatalieFeatures, ALCMMotionContextMotionState, MISSING_TYPE;

@interface ALMotionState : PBCodable
{
    double _timestamp;	// 8 = 0x8
    ALCMMotionContextMotionState *_motion;	// 16 = 0x10
    ALCLNatalieFeatures *_natalieFeatures;	// 24 = 0x18
    _Bool _regularEntry;	// 32 = 0x20
    CDStruct_b5306035 _has;	// 36 = 0x24
}

@property(nonatomic) _Bool regularEntry; // @synthesize regularEntry=_regularEntry;
@property(retain, nonatomic) ALCLNatalieFeatures *natalieFeatures; // @synthesize natalieFeatures=_natalieFeatures;
@property(retain, nonatomic) ALCMMotionContextMotionState *motion; // @synthesize motion=_motion;
@property(nonatomic) double timestamp; // @synthesize timestamp=_timestamp;
- (void)mergeFrom:(id)arg1;	// IMP=0x00100000002267c7
- (unsigned long long)hash;	// IMP=0x0010000000226662
- (_Bool)isEqual:(id)arg1;	// IMP=0x001000000022658b
- (id)copyWithZone:(struct _NSZone *)arg1;	// IMP=0x00100000002264b4
- (void)copyTo:(id)arg1;	// IMP=0x0010000000226436
- (void)writeTo:(id)arg1;	// IMP=0x00100000002263a1
- (_Bool)readFrom:(id)arg1;	// IMP=0x0010000000226394
- (id)dictionaryRepresentation;	// IMP=0x0010000000225f6a
- (id)description;	// IMP=0x0010000000225ef3
@property(readonly, nonatomic) _Bool hasNatalieFeatures;
@property(nonatomic) _Bool hasTimestamp;
- (MISSING_TYPE *)dealloc;	// IMP=0x0010000000225e40

@end

