//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <BaseBoard/BSSimpleAssertion.h>

@class BKSContextRelativePoint, MISSING_TYPE, NSString;

@interface BKMousePointerButtonDownRepositionAssertion : BSSimpleAssertion
{
    int _restrictingToPID;	// 8 = 0x8
    BKSContextRelativePoint *_contextRelativePointerPosition;	// 16 = 0x10
    MISSING_TYPE *_displayUUID;	// 24 = 0x18
}

- (void).cxx_destruct;	// IMP=0x0020000000034b72
@property(readonly, nonatomic) int restrictingToPID; // @synthesize restrictingToPID=_restrictingToPID;
@property(readonly, copy, nonatomic) NSString *displayUUID; // @synthesize displayUUID=_displayUUID;
@property(readonly, nonatomic) BKSContextRelativePoint *contextRelativePointerPosition; // @synthesize contextRelativePointerPosition=_contextRelativePointerPosition;
- (id)initWithIdentifier:(id)arg1 forReason:(id)arg2 contextRelativePointerPosition:(id)arg3 displayUUID:(id)arg4 restrictingToPID:(int)arg5 invalidationBlock:(CDUnknownBlockType)arg6;	// IMP=0x0010000000034a78

@end

