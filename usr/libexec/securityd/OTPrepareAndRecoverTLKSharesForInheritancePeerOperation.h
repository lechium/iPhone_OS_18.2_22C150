//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@class NSData, NSOperation, NSString, OTDeviceInformation, OTInheritanceKey, OTOperationDependencies, TPPolicyVersion, TrustedPeersHelperCustodianRecoveryKey;
@protocol OctagonStateString;

@interface OTPrepareAndRecoverTLKSharesForInheritancePeerOperation
{
    NSString<OctagonStateString> *_intendedState;	// 16 = 0x10
    NSString<OctagonStateString> *_nextState;	// 24 = 0x18
    unsigned long long _epoch;	// 32 = 0x20
    OTDeviceInformation *_deviceInfo;	// 40 = 0x28
    NSString *_peerID;	// 48 = 0x30
    NSData *;	// 56 = 0x38
    NSData *_permanentInfoSig;	// 64 = 0x40
    NSData *_stableInfo;	// 72 = 0x48
    NSData *_stableInfoSig;	// 80 = 0x50
    TPPolicyVersion *_policyOverride;	// 88 = 0x58
    OTOperationDependencies *_deps;	// 96 = 0x60
    OTInheritanceKey *_ik;	// 104 = 0x68
    NSString *_salt;	// 112 = 0x70
    NSOperation *_finishOp;	// 120 = 0x78
    TrustedPeersHelperCustodianRecoveryKey *_tphcrk;	// 128 = 0x80
}

- (void).cxx_destruct;	// IMP=0x002000000011faf4
@property(retain) TrustedPeersHelperCustodianRecoveryKey *tphcrk; // @synthesize tphcrk=_tphcrk;
@property(retain) NSOperation *finishOp; // @synthesize finishOp=_finishOp;
@property(retain) NSString *salt; // @synthesize salt=_salt;
@property(retain) OTInheritanceKey *ik; // @synthesize ik=_ik;
@property(retain) OTOperationDependencies *deps; // @synthesize deps=_deps;
@property(retain) TPPolicyVersion *policyOverride; // @synthesize policyOverride=_policyOverride;
@property(retain) NSData *stableInfoSig; // @synthesize stableInfoSig=_stableInfoSig;
@property(retain) NSData *stableInfo; // @synthesize stableInfo=_stableInfo;
@property(retain) NSData *permanentInfoSig; // @synthesize permanentInfoSig=_permanentInfoSig;
@property(retain) NSData *permanentInfo; // @synthesize permanentInfo=_permanentInfo;
@property(retain) NSString *peerID; // @synthesize peerID=_peerID;
@property(retain) OTDeviceInformation *deviceInfo; // @synthesize deviceInfo=_deviceInfo;
@property(nonatomic) unsigned long long epoch; // @synthesize epoch=_epoch;
@property(retain) NSString<OctagonStateString> *nextState; // @synthesize nextState=_nextState;
@property(readonly) NSString<OctagonStateString> *intendedState; // @synthesize intendedState=_intendedState;
- (void)proceedWithFilteredTLKShares:(id)arg1;	// IMP=0x001000000011f70e
- (void)groupStart;	// IMP=0x001000000011eeeb
- (id)initWithDependencies:(id)arg1 intendedState:(id)arg2 errorState:(id)arg3 ik:(id)arg4 deviceInfo:(id)arg5 policyOverride:(id)arg6 isInheritedAccount:(_Bool)arg7 epoch:(unsigned long long)arg8;	// IMP=0x001000000011ed66

@end

