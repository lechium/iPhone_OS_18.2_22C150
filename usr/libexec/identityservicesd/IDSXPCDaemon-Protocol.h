//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@class NSString;

@protocol IDSXPCDaemon
- (void)interalTestingCollaboratorWithCompletion:(void (^)(id <IDSXPCInternalTesting>, NSError *))arg1;
- (void)eventReportingCollaboratorWithCompletion:(void (^)(id <IDSXPCEventReporting>, NSError *))arg1;
- (void)baaSignerCollaboratorForTopic:(NSString *)arg1 withCompletion:(void (^)(id <IDSXPCBAASigner>, NSError *))arg2;
- (void)featureTogglerForService:(NSString *)arg1 withCompletion:(void (^)(id <IDSXPCFeatureToggler>, NSError *))arg2;
- (void)groupSessionKeyValueDeliveryProviderCollaboratorWithCompletion:(void (^)(id <IDSXPCGroupSessionKeyValueDeliveryProvider>, NSError *))arg1;
- (void)firewallCollaboratorForService:(NSString *)arg1 withCompletion:(void (^)(id <IDSXPCFirewall>, NSError *))arg2;
- (void)pinnedIdentityCollaboratorWithCompletion:(void (^)(id <IDSXPCPinnedIdentity>, NSError *))arg1;
- (void)keyTransparencyCollaboratorWithCompletion:(void (^)(id <IDSXPCKeyTransparency>, NSError *))arg1;
- (void)offGridStateManagerCollaboratorWithCompletion:(void (^)(id <IDSXPCOffGridStateManager>, NSError *))arg1;
- (void)offGridMessengerCollaboratorWithCompletion:(void (^)(id <IDSXPCOffGridMessenger>, NSError *))arg1;
- (void)serverMessagingCollaboratorWithCompletion:(void (^)(id <IDSXPCServerMessaging>, NSError *))arg1;
- (void)activityMonitorCollaboratorWithCompletion:(void (^)(id <IDSXPCActivityMonitor>, NSError *))arg1;
- (void)registrationCollaboratorWithCompletion:(void (^)(id <IDSXPCRegistration>, NSError *))arg1;
- (void)opportunisticCollaboratorWithCompletion:(void (^)(id <IDSXPCOpportunistic>, NSError *))arg1;
- (void)reunionSyncCollaboratorForClientProcessName:(NSString *)arg1 withCompletion:(void (^)(id <IDSXPCReunionSync>, NSError *))arg2;
- (void)pairedDeviceManagerCollaboratorWithCompletion:(void (^)(id <IDSXPCPairedDeviceManager>, NSError *))arg1;
- (void)pairingCollaboratorWithCompletion:(void (^)(id <IDSXPCPairing>, NSError *))arg1;
- (void)activateWithCompletion:(void (^)(NSError *))arg1;
@end

